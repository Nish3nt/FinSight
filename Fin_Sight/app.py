# =============================================================================
#  FinSight — app.py  (UI Entry Point)
#  Run: streamlit run app.py
#  Requires backend.py in same directory
# =============================================================================

# backend.py auto-installs all packages before any other imports
from backend import (
    call_groq, get_news, compute_vader, fetch_stock_data,
    compute_features, compute_drawdown, train_model, GROQ_MODEL
)
...
.rec-box-buy{background:#052e16;border:1px solid #166534;
             border-radius:12px;padding:18px 20px;margin-top:10px}
.rec-box-hold{background:#1c1917;border:1px solid #78350f;
              border-radius:12px;padding:18px 20px;margin-top:10px}
...
# ==============================================================================
    ...
    future_dates=pd.date_range(start=data_main.index[-1]+pd.Timedelta(days=1),
                                periods=days,freq='B')
    ...
    fig_f.add_trace(go.Scatter(x=[future_dates[0],future_dates[0]],
                               y=[lowers[0],uppers[0]],fill='toself',
    ...
    if st.button("Generate AI Analysis", type="primary", key="pred_ai"):
        ...
        with st.spinner("Analysing with Llama 3.3 70B..."):
            prompt=f"""You are a professional stock analyst. Analyse this LSTM forecast.
Stock: {selected_ticker} | Current: ${last_price:.2f}
Forecast {days} days: ${final_price:.2f} ({pct_change:+.2f}%)
Range: Low ${lowers[-1]:.2f} - High ${uppers[-1]:.2f}
Model: DA={art['da']:.1f}%, MAPE={art['mape']:.2f}%, WF-R2={art['wf_r2']:.3f}, Beats Naive={wins}/3
Give 3 sections: 1.FORECAST INTERPRETATION 2.MODEL RELIABILITY 3.KEY RISKS (bullets)
Be direct and quantitative."""
            response=call_groq(prompt,max_tokens=500)
        if response.startswith("__ERROR__"):
            st.error(f"AI failed: {response}")
        else:
...
# ==============================================================================
    ...
    ra_m=rm/vm if vm>0 else 0; ra_c=rc/vc if vc>0 else 0
    ...
    with cc2:
        ...
        fig4=go.Figure()
        fig4.add_trace(go.Scatter(x=[vm,vc],y=[rm,rc],mode='markers+text',
                                  text=[selected_ticker,compare_ticker],
                                  textposition='top center',
                                  marker=dict(size=22,color=['#26A69A','#AB47BC'],
                                              symbol='diamond')))
    ...
    st.caption(f"Powered by {GROQ_MODEL}")
    if st.button("Get AI Recommendation",type="primary",key="comp_ai"):
        with st.spinner("Analysing with Llama 3.3 70B..."):
            prompt=f"""You are a senior equity analyst. Compare these two stocks and give a clear recommendation.
{selected_ticker}: Return={rm:+.2f}%, Vol={vm:.1f}%, MaxDD={max_dd_m:.1f}%, RiskAdj={ra_m:.2f}x
{compare_ticker}: Return={rc:+.2f}%, Vol={vc:.1f}%, MaxDD={max_dd_c:.1f}%, RiskAdj={ra_c:.2f}x
Period: {start_date} to {end_date}
Format:
RECOMMENDATION: [ticker] is the stronger pick
REASON (3 bullets with numbers):
- ...
WHO SHOULD CONSIDER {selected_ticker}: [1 sentence]
WHO SHOULD CONSIDER {compare_ticker}: [1 sentence]
RISK WARNING: [1 sentence]
Be decisive, cite numbers."""
            response=call_groq(prompt,max_tokens=500)
        if response.startswith("__ERROR__"):
            st.error(f"AI failed: {response}")
        else:
            box_cls="rec-box-buy" if selected_ticker.upper() in response.upper()[:80] \
                    else "rec-box-hold"
...
# ==============================================================================
    ...
    m_ret=rets.mean()*252; cov_mat=rets.cov()*252
    w_np=np.array(weights); n_assets=len(port_tickers)
    ...
    dd_port=(port_idx-port_idx.cummax())/port_idx.cummax()*100
    ...
    st.caption("3,000 random weight combinations. Brighter = better Sharpe. Red = yours. Gold = optimal.")
    n_sim=3000; s_rets,s_vols,s_sharpes,s_wts=[],[],[],[]
    for _ in range(n_sim):
        w_=np.random.dirichlet(np.ones(n_assets))
        r_=float(np.dot(m_ret,w_)); v_=float(np.sqrt(np.dot(w_.T,np.dot(cov_mat,w_))))
    ...
    fig_ef=go.Figure()
    fig_ef.add_trace(go.Scatter(x=s_vols,y=s_rets,mode='markers',
                                marker=dict(size=4,color=s_sharpes,
                                            colorscale='Viridis',showscale=True,
                                            colorbar=dict(title='Sharpe')),
                                text=hover,
                                hovertemplate='Return:%{y:.1f}%<br>Risk:%{x:.1f}%<br>%{text}<extra></extra>',
                                name='Simulated'))
    fig_ef.add_trace(go.Scatter(x=[p_vol*100],y=[p_ret*100],mode='markers+text',
                                marker=dict(size=18,color='red',symbol='star'),
                                text=['Your Portfolio'],textposition='top center',
                                name='Yours'))
    best_idx=int(np.argmax(s_sharpes))
    fig_ef.add_trace(go.Scatter(x=[s_vols[best_idx]],y=[s_rets[best_idx]],
                                mode='markers+text',
                                marker=dict(size=18,color='#fbbf24',symbol='star'),
                                text=['Optimal'],textposition='top center',
                                name='Max Sharpe'))
    fig_ef.update_layout(height=460,xaxis_title="Risk (Volatility %)",
    ...
    st.markdown(f"<div class='ins-card'>Optimal weights (Sharpe {s_sharpes[best_idx]:.2f}): "
                f"<b>{opt_hint}</b>. Your current Sharpe: <b>{sharpe:.2f}</b>.</div>",
                unsafe_allow_html=True)

    # Monte Carlo
    st.markdown("#### Monte Carlo Simulation — 1-Year Outlook")
    st.caption("500 simulated paths from $100 starting value over 252 trading days.")
    n_mc=500; n_days_mc=252; start_val=100.0
    daily_mean=float(port_daily.mean()); daily_std=float(port_daily.std())
    ...
    for i in range(0,n_mc,5):
        fig_mc.add_trace(go.Scatter(x=list(range(n_days_mc)),y=mc_paths[i],
                                    mode='lines',
    ...
    for path,name,color,width in [
        ...
        fig_mc.add_trace(go.Scatter(x=list(range(n_days_mc)),y=path,
                                    name=name,line=dict(color=color,width=width)))
    fig_mc.update_layout(height=380,xaxis_title="Trading Days",
    ...
    if st.button("Generate AI Portfolio Insights",type="primary",key="port_ai"):
        ...
        with st.spinner("Analysing with Llama 3.3 70B..."):
            prompt=f"""You are a senior portfolio manager. Give 4 specific data-driven insights.
Portfolio: {', '.join([f"{port_tickers[i]} {weights[i]*100:.1f}%" for i in range(n_assets)])}
Return={p_ret*100:.2f}%, Vol={p_vol*100:.2f}%, Sharpe={sharpe:.2f}, Sortino={sortino:.2f}
Beta={beta:.2f}, MaxDD={max_dd_p:.1f}%, VaR95={var_95:.2f}%, CVaR={cvar_95:.2f}%
Most correlated: {max_pair[0]}&{max_pair[1]} r={max_pair[2]:.2f}
Optimal weights: {opt_hint} (Sharpe {s_sharpes[best_idx]:.2f}) vs current {sharpe:.2f}
MC median 1Y: ${p50_:.2f}, worst 5%: ${p5_:.2f}
Format exactly:
INSIGHT 1: [Title]
[2-3 sentences with numbers and one action]
INSIGHT 2: [Title]
[2-3 sentences with numbers and one action]
INSIGHT 3: [Title]
[2-3 sentences with numbers and one action]
