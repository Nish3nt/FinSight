...
#  Requires: backend.py in the same directory
# =============================================================================
...
from backend import (
    ...
    compute_features, compute_drawdown, train_model,
    GROQ_MODEL
)
...
# =============================================================================
#  LOAD DATA & NEWS (runs on every page load)
# =============================================================================
...
# =============================================================================
    ...
    st.caption("Naive = tomorrow equals today. Any useful model must beat this.")
    ...
    if art['wf_list']:
        ...
        fig_wf = go.Figure()
        for fn, fv, fc in zip(
                [f"Fold {i+1}" for i in range(len(art['wf_list']))],
                art['wf_list'], wfc):
        ...
        fig_wf.update_layout(
            yaxis_range=[min(min(art['wf_list'])-0.05, -0.05), 1.05],
            height=300)
    ...
    for step in range(days):
        ...
        rs_    = (adj_s.rolling(20).std().iloc[-1]
                  if len(adj_s) >= 20 else adj_s.std())
    ...
    future_df['Change %'] = [
        f"{(p - last_price) / last_price * 100:+.2f}%"
    ...
    fig_f.add_trace(go.Scatter(
        x=[future_dates[0]], y=[future_preds_price[0]],
    ...
    if st.button("Generate AI Analysis of Forecast", type="primary",
                 key="pred_ai"):
...
Model Quality:
- Directional Accuracy: {art['da']:.1f}% (random=50%)
- MAPE: {art['mape']:.2f}% (avg price error)
- Walk-Forward R2: {art['wf_r2']:.3f} (1.0=perfect)
- RMSE: ${art['rmse']:.2f}
- Beats Naive Baseline: {wins}/3 metrics

Provide exactly 3 sections:
1. FORECAST INTERPRETATION: What does this prediction mean?
2. MODEL RELIABILITY: How much should a trader trust this?
3. KEY RISKS: 2-3 bullet points of what could invalidate this.

Be direct, quantitative, professional."""
...
# =============================================================================
    ...
    if news_posts:
        ...
        st.dataframe(
            df_s.style.map(color, subset=['Score'])
                      .format({'Score': '{:.3f}'}),
            use_container_width=True)
...
# =============================================================================
#  TAB 4 — COMPARISON  (+ AI Recommendation)
# =============================================================================
elif tab == "Comparison":
    ...
    st.caption(f"Powered by {GROQ_MODEL} — tells you which stock to pick and why")
...
REASON (3 bullet points with specific numbers):
- [Point 1]
- [Point 2]
- [Point 3]
...
RISK WARNING: [1 sentence about what could change this]

Be decisive and cite the numbers."""
...
# =============================================================================
    ...
    st.caption("SPY is the standard benchmark used by every professional fund manager.")
    fig_b = go.Figure()
    ...
    st.caption("3,000 random weight combos. Brighter = better Sharpe. Red star = yours. Gold star = optimal.")
    n_sim = 3000
    ...
    st.caption("500 simulated paths from $100. Shows realistic range of outcomes over 252 trading days.")
    ...
    for i in range(0, n_mc, 5):
        fig_mc.add_trace(go.Scatter(
            x=list(range(n_days_mc)), y=mc_paths[i], mode='lines',
    ...
    # ── Individual Stock Analytics Table ───────────────────────────────────────
    ...
    st.caption(f"Powered by {GROQ_MODEL} via Groq — professional portfolio analysis in seconds")
...
Portfolio:
{chr(10).join([f"- {port_tickers[i]}: {weights[i]*100:.1f}%" for i in range(n_assets)])}
...
- Optimal weights: {opt_hint} (Sharpe {s_sharpes[best_idx]:.2f})
- Current Sharpe: {sharpe:.2f}
- Monte Carlo median 1Y from $100: ${p50_:.2f}
- Worst case 5%: ${p5_:.2f}
...
# =============================================================================
#  NEWS TICKER — appears on every tab at the bottom
# =============================================================================
st.markdown("---")
